...
""" dataset config """
parser.add_argument('--dataset', type=str, default='cifar10', choices=['cifar10', 'imagenet', 'imagenet10', 'imagenet100'])
...
if __name__ == '__main__':
    ...
    if args.model=="vgg":
        ...
        if args.base_path!=None:
            ...
            net_origin = nn.DataParallel(VGG_CIFAR(num_classes=run_config.data_provider.n_classes))
        ...
        if args.base_path!=None:
            weight_path = args.base_path+'/checkpoint/model_best.pth.tar'
            net_origin = nn.DataParallel(ResNet_CIFAR(depth=56, num_classes=run_config.data_provider.n_classes))
    elif args.model=="resnet110":
        assert args.dataset=='cifar10', 'resnet56 only supports cifar10 dataset'
        net = ResNet_CIFAR(depth=110, num_classes=run_config.data_provider.n_classes, cfg=eval(args.cfg))
        if args.base_path!=None:
            weight_path = args.base_path+'/checkpoint/model_best.pth.tar'
            net_origin = nn.DataParallel(ResNet_CIFAR(depth=110, num_classes=run_config.data_provider.n_classes))
